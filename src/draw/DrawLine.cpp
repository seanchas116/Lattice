...
void DrawLine::draw(const SP<GL::VAO> &vao, const glm::dmat4 &matrix, const SP<OldCamera> &camera, double width, glm::vec4 color, bool useVertexColor, double zOffset) {
    _shader.bind();
    ...
    _shader.setUniform("zNear", camera->projection() == OldCamera::Projection::Perspective ? camera->zNear() : -10000.0); // TODO: specify depth in better way
