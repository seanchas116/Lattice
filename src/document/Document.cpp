    ...
    _history(makeShared<History>())
{
    ...
    connect(this, &Document::editingItemChanged, this, [this] {
...
void Document::setEditingItem(const std::optional<SP<MeshItem> > &item) {
    if (item != _editingItem) {
        _editingItem = item;
        emit editingItemChanged(item);
    }
}
